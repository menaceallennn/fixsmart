...
const expensiveZipPrefixes = ["90","91","92","93","94","95","96","97","98","99","10","11","20","21","22","70","71","80","81"];
...
function getBrandFactor(make) {
  ...
  if (luxuryBrands.some(brand => m.includes(brand))) {
    return { factor: 1.35, label: "Luxury brand", note: "Luxury and premium brands usually cost more for parts and labor." };
  }
  if (economyBrands.some(brand => m.includes(brand))) {
    return { factor: 1.00, label: "Standard brand", note: "Common brands usually have normal parts and labor pricing." };
  }
  return { factor: 1.12, label: "Less common brand", note: "Less common vehicles can cost more because parts or shop familiarity may vary." };
}
...
function getAgeFactor(year, engineType) {
  ...
  let note = "Normal age factor applied.";

  if (age >= 45) {
    factor = 1.35;
    label = "Classic vehicle";
    note = "Very old cars may need harder-to-find parts and extra labor.";
  } else if (age >= 25) {
    factor = 1.22;
    label = "Older vehicle";
    note = "Older vehicles can need more labor due to rust, worn bolts, or part availability.";
  } else if (age >= 15) {
    factor = 1.12;
    label = "High age vehicle";
    note = "Age can increase labor because parts and fasteners may be worn.";
  } else if (age <= 3) {
    factor = 1.08;
    label = "Newer vehicle";
    note = "Newer cars may have more electronics and higher part pricing.";
  }
...
function getMileageFactor(mileage) {
  ...
  if (mi >= 200000) return { factor: 1.18, label: "Very high mileage", note: "Very high mileage increases the chance of related worn parts." };
  if (mi >= 150000) return { factor: 1.12, label: "High mileage", note: "High mileage can raise labor and diagnostic risk." };
  if (mi >= 90000) return { factor: 1.06, label: "Moderate mileage", note: "Some wear-related issues become more common." };
  if (mi > 0 && mi < 30000) return { factor: 1.03, label: "Low mileage", note: "Low mileage keeps wear lower but parts can still fail." };

  return { factor: 1.00, label: "Normal mileage", note: "No major mileage adjustment applied." };
}
...
function getEngineFactor(engineType, issueKey) {
  ...
  if (engineType === "turbo" && ["slowAcceleration", "shaking", "checkEngine"].includes(issueKey)) {
    factor += 0.08;
  }

  return { factor, label: titleCase(engineType), note: "Engine type changes diagnostic time parts and labor complexity." };
}

function getConditionFactor(condition) {
  const map = {
    clean: { factor: 0.97, label: "Clean / maintained", note: "A maintained car can slightly reduce extra repair risk." },
    average: { factor: 1, label: "Average condition", note: "Normal condition factor applied." },
    rough: { factor: 1.18, label: "Rough condition", note: "Neglected cars can need extra labor or related repairs." },
    modified: { factor: 1.20, label: "Modified vehicle", note: "Modified vehicles can take longer to diagnose and repair." }
  };
...
function getLocationFactor(zip) {
  ...
  if (z.startsWith("799") || z.startsWith("88")) {
    return { factor: 0.96, label: "El Paso area", note: "El Paso TX is included with a slightly lower labor market factor." };
  }

  if (expensiveZipPrefixes.some(prefix => z.startsWith(prefix))) {
    return { factor: 1.12, label: "Higher labor market", note: "This ZIP may have higher average labor pricing." };
  }

  if (z.startsWith("78") || z.startsWith("77") || z.startsWith("75") || z.startsWith("76")) {
    return { factor: 1.00, label: "Texas market", note: "Normal Texas regional labor pricing applied." };
  }

  return { factor: 1.05, label: "General US market", note: "Small location adjustment applied when local pricing is unknown." };
}
...
function getVerdict(calc) {
  ...
  if (quote < min * 0.65) {
    return { label: "VERY LOW", type: "warn", desc: "This quote is unusually low. Make sure it includes parts labor diagnostics taxes and the full repair.", risk: "Low quote risk", angle: -20 };
  }

  if (quote <= max * 1.15) {
    return { label: "FAIR", type: "good", desc: "Your quote is within the expected adjusted range. Still ask for parts labor warranty and diagnostic proof.", risk: "Low price risk", angle: -42 };
  }

  if (quote <= max * 1.65) {
    return { label: "HIGH", type: "warn", desc: "Your quote is above the expected adjusted range. Ask for a full breakdown and consider a second opinion.", risk: "Medium price risk", angle: 0 };
  }

  return { label: "OVERPRICED", type: "bad", desc: "Your quote is much higher than the adjusted range. Get a second opinion before approving the repair.", risk: "High price risk", angle: 48 };
}
...
function renderAdjustments(calc) {
  ...
  const items = [
    ...
    { name: "Total factor", value: calc.totalFactor, note: "Base repair range multiplied by all vehicle factors." },
...
function adjustedCause(cause, factor) {
  return {
    ...cause,
    min: Math.max(0, cause.min * factor),
    max: Math.max(cause.min * factor + 20, cause.max * factor)
  };
}
...
function renderCards(calc, verdict) {
  ...
  document.getElementById("scoreText").textContent =
    calc.score >= 70
      ? "This repair is usually worth fixing if the quote is fair."
      : calc.score >= 45
        ? "This repair may be worth doing but compare quotes and confirm diagnosis."
        : "High risk repair. Get a second opinion and compare against the vehicle value.";
  ...
  if (calc.issue.urgency === "High") {
    redFlags.innerHTML += "<li>High urgency issue. Do not delay inspection.</li>";
  }
